# Hi, I'm Amine Harkane 👋

## Data Engineer | Business Intelligence Specialist | Big Data & Cloud Solutions

🎓 **Master's in Big Data Engineering** (Final Year) | 📍 Based in France  
💼 **Open to opportunities:** BI Engineer | Data Engineer | Big Data Engineer roles (Internships & Full-time)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/aharkane)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:harkaneamine@gmail.com)
[![Microsoft Learn](https://img.shields.io/badge/Microsoft_Learn-0078D4?style=flat&logo=microsoft&logoColor=white)](https://learn.microsoft.com/en-us/users/amineharkane-6987/)

---

## 🚀 About Me

Data Engineering graduate with **2+ years of hands-on experience** building production-grade data pipelines and BI solutions. Specialized in designing scalable data architectures using Microsoft Azure, SQL Server, and modern Big Data technologies.

**What I bring:**
- ✅ **Production Experience** – Delivered end-to-end BI solutions for luxury retail and public administration clients
- ✅ **Microsoft Certified** – DP-203 (Azure Data Engineer) & PL-300 (Power BI Data Analyst)
- ✅ **Full Stack Data Engineering** – From data ingestion and ETL to visualization and real-time analytics
- ✅ **Problem Solver** – Proven track record collaborating with cross-functional teams to transform complex data into actionable insights

**Currently exploring:** Distributed computing patterns, real-time streaming analytics, and MLOps practices

---

## 🛠️ Technical Skills

### Big Data & Distributed Computing
![Spark](https://img.shields.io/badge/Apache_Spark-E25A1C?style=flat&logo=apachespark&logoColor=white)
![PySpark](https://img.shields.io/badge/PySpark-E25A1C?style=flat&logo=apachespark&logoColor=white)
![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=flat&logo=databricks&logoColor=white)
![dbt](https://img.shields.io/badge/dbt-FF694B?style=flat&logo=dbt&logoColor=white)

### Cloud & Data Platforms
![Azure](https://img.shields.io/badge/Microsoft_Azure-0078D4?style=flat&logo=microsoftazure&logoColor=white)
![Fabric](https://img.shields.io/badge/Microsoft_Fabric-0078D4?style=flat&logo=microsoft&logoColor=white)
![Synapse](https://img.shields.io/badge/Azure_Synapse-0078D4?style=flat&logo=microsoftazure&logoColor=white)
![BigQuery](https://img.shields.io/badge/BigQuery-4285F4?style=flat&logo=googlebigquery&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonaws&logoColor=white)

### Databases & ETL
![SQL Server](https://img.shields.io/badge/SQL_Server-CC2927?style=flat&logo=microsoftsqlserver&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)
![SSIS](https://img.shields.io/badge/SSIS-CC2927?style=flat&logo=microsoft&logoColor=white)
![SSAS](https://img.shields.io/badge/SSAS-CC2927?style=flat&logo=microsoft&logoColor=white)

### Analytics & Visualization
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=flat&logo=powerbi&logoColor=black)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat&logo=tableau&logoColor=white)
![DAX](https://img.shields.io/badge/DAX-F2C811?style=flat&logo=powerbi&logoColor=black)

---

## 🏆 Certifications

| Certification | Badge | Verify |
|---------------|-------|--------|
| **Azure Data Engineer Associate** (DP-203) | ![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat&logo=microsoftazure&logoColor=white) | [View Credential](https://learn.microsoft.com/en-us/users/amineharkane-6987/credentials/19a5370ba9092706) |
| **Power BI Data Analyst Associate** (PL-300) | ![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?style=flat&logo=powerbi&logoColor=black) | [View Credential](https://learn.microsoft.com/en-us/users/amineharkane-6987/credentials/6fa9f192fce1221c) |
| **Databricks Fundamentals** | ![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=flat&logo=databricks&logoColor=white) | [View Credential](https://credentials.databricks.com/97a08616-9404-430a-a137-6262c5756816) |
| **dbt Fundamentals** | ![dbt](https://img.shields.io/badge/dbt-FF694B?style=flat&logo=dbt&logoColor=white) | [View Credential](https://credentials.getdbt.com/12d0afd7-8aa6-4fac-87be-2bb03644465d) |
| **Google Data Analytics Professional** | ![Google](https://img.shields.io/badge/Google-4285F4?style=flat&logo=google&logoColor=white) | [View Credential](https://www.coursera.org/account/accomplishments/professional-cert/2YYTBUFBJFAX) |

---

## 💼 Featured Projects

### 🏢 [Enterprise Data Warehouse - Metadata-Driven Architecture](https://github.com/aharkane/adventureworks-metadata_driven_data_warehouse_implementation)
**Production-scale ETL system with automated dimensional modeling**

`SQL Server` `SSIS` `T-SQL` `Dimensional Modeling`

- Processes **120K+ records** with sub-5-second incremental loads
- Implements **19 stored procedures** with SCD Type 0/1/2 patterns
- Full CDC integration, audit framework, and star schema design

### 🏭 [Microsoft Fabric Lakehouse Implementation](https://github.com/aharkane/ms-fabric-dwh_implementation)
**Modern data architecture combining Lakehouse and Data Warehouse patterns**

`Microsoft Fabric` `Delta Lake` `T-SQL` `Serverless Compute`

- Unified analytics platform with automated data pipelines
- Supports customer analytics, product performance tracking, and business metrics
- Leverages serverless compute for cost optimization

### ☁️ [Azure Synapse + PySpark Data Engineering](https://github.com/aharkane/azure-synapse_sql-pyspark_data_engineering)
**Distributed big data processing solution for multi-format data at scale**

`Azure Synapse` `PySpark` `Delta Lake` `ADLS Gen2`

- Advanced partitioning strategies and schema evolution
- ACID transactions with Delta Lake time travel capabilities
- Optimized query performance through partition pruning and predicate pushdown

### 📊 [Retail Analytics - 2.1M+ Transaction Analysis](https://github.com/aharkane/retail-analytics-pandas)
**Enterprise-scale data analysis for retail M&A due diligence**

`Python` `Pandas` `NumPy` `Data Quality Engineering`

- Analyzed **2.1M+ transactions** across 84K+ products
- Achieved **25% memory optimization** through efficient data processing
- Comprehensive data quality validation and business insights

### 🚴 [Capital Bikeshare Customer Analytics Dashboard](https://public.tableau.com/app/profile/mohamed.amine.harkane/viz/UrbanBikeSharinginWashingtonDCNeighbourhood/UrbanBikeSharinginWashingtongDCNeighbourhood)
**Interactive geospatial analytics for customer behavior optimization**

`Tableau` `SQL` `Geospatial Analytics`

- Customer segmentation and conversion funnel analysis
- Interactive route mapping for operational planning
- Data-driven recommendations for membership growth

### 📈 [AdventureWorks Sales Performance Dashboard](https://app.powerbi.com/view?r=eyJrIjoiMzlhOGMyMGItZjdkNS00NzgwLWJkNmMtNmZkYjVmMTg4NDg0IiwidCI6IjBhNmU5Njk2LWE1ZDktNDFiOS04MDhmLWI4YTQ1YjJhMmI2MyJ9)
**Real-time business intelligence solution with KPI tracking**

`Power BI` `DAX` `T-SQL` `Interactive Analytics`

- Live dashboard for sales performance monitoring
- Advanced DAX calculations for seller insights
- [**📊 View Live Dashboard**](https://app.powerbi.com/view?r=eyJrIjoiMzlhOGMyMGItZjdkNS00NzgwLWJkNmMtNmZkYjVmMTg4NDg0IiwidCI6IjBhNmU5Njk2LWE1ZDktNDFiOS04MDhmLWI4YTQ1YjJhMmI2MyJ9)

<details>
<summary><b>📦 More ETL & Data Pipeline Projects</b></summary>

#### [SSIS Product Data Pipeline with SCD Type 2](https://github.com/aharkane/adventureworks-dwh-csv_products_scd_data_etl)
**Historical change tracking for product catalog management**
- Dual processing approaches (row-by-row and batch optimization)
- Complete audit trail with IsCurrent flags and validity dates

#### [SSIS Database Export Pipeline](https://github.com/aharkane/adventureworks-dwh-db_to_csv_data_export)
**Automated data extraction using command-line utilities**
- Metadata-driven table enumeration with BCP.EXE and SQLCMD.EXE
- Intelligent folder routing for dimensional and fact tables

#### [AdventureWorks SQL Analytics](https://github.com/aharkane/adventureworks-sql-analytics)
**Advanced T-SQL patterns for business reporting**
- Window functions, CTEs, hierarchical queries
- Product analysis, customer intelligence, transaction management

#### [Financial Reporting ETL Pipeline](https://github.com/aharkane/courtiers-commissions-reporting)
**Automated commission processing for 7+ insurance brokers**
- Python-based ETL with dynamic Excel report generation
- Streamlined financial operations workflow

</details>

---

## 📫 Let's Connect

I'm always interested in discussing data engineering challenges, cloud architecture, or collaboration opportunities.

- 💼 **LinkedIn:** [linkedin.com/in/aharkane](https://www.linkedin.com/in/aharkane)
- 📧 **Email:** harkaneamine@gmail.com
- 📚 **Microsoft Learn:** [View Profile](https://learn.microsoft.com/en-us/users/amineharkane-6987/)

---

<div align="center">
  <i>⭐ If you find my projects interesting, feel free to star them!</i>
</div>
